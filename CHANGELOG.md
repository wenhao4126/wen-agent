...
All notable changes to the Go line (Wenhao 1.0+) are recorded here. The legacy
`0.x` TypeScript history lives on the [`v1`](https://github.com/esengine/DeepSeek-Wenhao/tree/v1)
branch.
...
- **Go kernel**: a single static binary (CGO-free), cross-compiled for
  ...
  the native binary), Homebrew (`esengine/wenhao` tap), and release archives;
...
- **Code intelligence via CodeGraph**: a tree-sitter symbol/call graph
  ...
  or API cost. Fetched into a local cache on first use (or `wenhao codegraph
...
- **Memory**: `WENHAO.md` hierarchy + auto-memory, folded into the cache-stable
  prefix.
- **ACP** (`wenhao acp`) and an HTTP/SSE server frontend; desktop app (Wails).
...
[1.0.0]: https://github.com/esengine/DeepSeek-Wenhao/releases/tag/v1.0.0
