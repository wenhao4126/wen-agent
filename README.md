<p align="center">
  <img src="docs/logo.svg" alt="Wenhao" width="640"/>
</p>
...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <a href="https://esengine.github.io/DeepSeek-Wenhao/">Website</a>
  &nbsp;·&nbsp;
  <strong><a href="https://discord.gg/XF78rEME2D">Discord</a></strong>
</p>

> [!IMPORTANT]
> **Wenhao 1.0 is a ground-up rewrite in Go** — this branch (`main-v2`) is the new default and where development happens now.
...
- **Config-driven.** Providers, the agent, enabled tools, and plugins are all
  declared in `wenhao.toml`. No hardcoded models.
...
```sh
npm i -g wenhao                  # any OS; pulls the prebuilt native binary
brew install esengine/wenhao/wenhao   # macOS
```

Prebuilt archives (`darwin|linux|windows × amd64|arm64`) and `SHA256SUMS` are on
every [GitHub release](https://github.com/esengine/DeepSeek-Wenhao/releases).
...
```sh
wenhao setup                      # config wizard → ./wenhao.toml
export DEEPSEEK_API_KEY=sk-...  # or put it in .env (see .env.example)
wenhao chat                       # then run /init to generate AGENTS.md (project memory)
wenhao run "implement the TODOs in main.go"
wenhao run --model mimo-pro "add unit tests for this function"
echo "explain this code" | wenhao run
```

## Configuration

Resolution order: **flag > `./wenhao.toml` > `~/.config/wenhao/config.toml` >
...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $WENHAO_LANG
...
always allow; writers fall back to `mode`). `wenhao chat` prompts before writers
(`y` once · `a` this session · `n` no); `wenhao run` stays autonomous but still
...
Wenhao is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
exposes. `make build` also produces `bin/wenhao-plugin-example` — a runnable
...
**Upgrading from `0.x`?** Your old `~/.wenhao/config.json` is still read for its
`mcpServers` (honouring `mcpDisabled`) as a lowest-priority source, so MCP servers
keep working — move them into `wenhao.toml`'s `[[plugins]]` or a `.mcp.json` when
convenient.

### Slash commands

In `wenhao chat`, built-in commands (`/compact`, `/new`, `/rewind`, `/tree`,
...
`.wenhao/commands/` (project) or `~/.config/wenhao/commands/` (user) —
...
Embed `@` references in a message and Wenhao resolves them before sending, as
...
`wenhao setup` keeps first-run minimal: pick provider → keys (every SKU of a
...
The planner sees loaded `WENHAO.md` / `AGENTS.md` memory and a small read-only
...
automatically: Wenhao first drafts a read-only plan, then waits for approval
...
`/auto-plan off|on` in `wenhao chat` to change the user-level setting, or
`wenhao config auto-plan off|on` from a shell/script. Pass `--local` to the
...
TOML config, an interactive `wenhao setup` wizard, two-model collaboration
...
or a project `.mcp.json` — custom slash commands (`.wenhao/commands/*.md`),
`@file` / `@resource` references, plus a runnable reference plugin
(`cmd/wenhao-plugin-example`), the harness loop, and CLI. A Wails desktop
...
<a href="https://www.star-history.com/?repos=esengine%2FDeepSeek-Wenhao&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=esengine/DeepSeek-Wenhao&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=esengine/DeepSeek-Wenhao&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=esengine/DeepSeek-Wenhao&type=date&legend=top-left" />
 </picture>
</a>
...
If Wenhao has been useful and you'd like to say thanks, you can. It stays a coffee, not a contract — donations don't buy feature priority or change how issues get triaged.
...
A small list of folks whose work has shaped Wenhao the most — measured
...
<p align="center">
  <a href="https://github.com/esengine/DeepSeek-Wenhao/graphs/contributors">
    <img src="https://contrib.rocks/image?repo=esengine/DeepSeek-Wenhao&max=100&columns=12" alt="Contributors to esengine/DeepSeek-Wenhao" width="860"/>
  </a>
</p>
