...
> [!IMPORTANT]
> **Wenhao 1.0 是用 Go 从零重写的版本** —— 本分支(`main-v2`)已是新的默认分支,后续开发都在这里。
...
- **配置驱动**：provider、agent、启用的工具、插件全部在 `wenhao.toml` 中声明，
  内核无硬编码模型。
...
```sh
npm i -g wenhao                  # 任意系统;自动拉取对应平台的原生二进制
...
`mode`）。`wenhao chat` 会在 writer 调用前征求同意（`1` 本次 · `2` 本会话 · `3` 拒绝，兼容 `y/a/n`）；
`wenhao run` 保持自主运行但仍然遵守 `deny`。完整 schema 与契约见
...
Wenhao 是一个 MCP 客户端。`[[plugins]]` 的 `type` 选择传输：`stdio`（默认）启动本地子进
...
各自暴露的内容。`make build` 还会产出 `bin/wenhao-plugin-example`——一个可直接运行的
...
**已有 Claude Code 的 `.mcp.json`？** 直接放到项目根目录，Wenhao 会原样读取——其
`mcpServers` 规范（`command`/`args`/`env`、`type`/`url`/`headers`、`${VAR}` 展开）
与 `[[plugins]]` 字段一一对应。两处来源会合并加载；同名时以 `wenhao.toml` 为准。
...
**从 `0.x` 升级？** 旧的 `~/.wenhao/config.json` 仍会被读取(读其 `mcpServers`、并遵从
`mcpDisabled`),作为最低优先级来源——所以 MCP 服务器照常可用;方便时再把它们挪进
`wenhao.toml` 的 `[[plugins]]` 或 `.mcp.json`。

### 斜杠命令

`wenhao chat` 里,内置命令(`/compact`、`/new`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/effort`、`/mcp`、`/help`)在本地执行。
`/tree` 查看已保存的对话分支,`/branch [name]` 从当前对话末端分支,`/branch <turn> [name]`
从较早的 checkpoint 轮次分支,`/switch <id|name>` 切换到另一个分支。**自定义命令**
是放在 `.wenhao/commands/`(项目)或 `~/.config/wenhao/commands/`(用户)下的 Markdown 文件——
...
在消息里写 `@` 引用,Wenhao 会在发送前解析成带标签的上下文块:`@path/to/file`(或
...
`wenhao setup` 刻意保持首次体验极简：选 provider → 输入 key（所选 provider 的所有
SKU 都会启用）。若要让两个模型协同（执行器 + 规划器，各自独立、缓存稳定的
session），向导后手动在 `wenhao.toml` 加一行即可：
...
Planner 会看到已加载的 `WENHAO.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
...
的任务会自动进入 plan mode：Wenhao 先只读生成计划，待用户批准后才
编辑文件或执行有副作用的命令。`auto_plan_classifier` 可以指定便宜的 provider，例如
`deepseek-flash`；它只在边界输入上调用，分类失败会回退到启发式规则。也可以用
`wenhao chat` 里的 `/auto-plan off|on` 修改用户级设置，或在 shell/脚本里用
`wenhao config auto-plan off|on`。只有明确想写项目级覆盖时，才给 shell 命令加
`--local`。
...
grep、web_fetch）、TOML 配置、交互式 `wenhao setup` 向导、双模型协同（执行器 + 规划器，
...
（`.wenhao/commands/*.md`）、`@file` / `@resource` 引用、外加可运行的参考插件
（`cmd/wenhao-plugin-example`）、harness 主循环、CLI。chat 在终端普通缓冲区运行(原生
...
如果 Wenhao 帮你省了时间或 token，欢迎请杯咖啡。捐助不会换来 feature 优先级，也不会影响 issue 的处理顺序——就是「谢谢」。
...
<p align="center">
  <a href="https://github.com/esengine/DeepSeek-Wenhao/graphs/contributors">
    <img src="https://contrib.rocks/image?repo=esengine/DeepSeek-Wenhao&max=100&columns=12" alt="esengine/DeepSeek-Wenhao 贡献者" width="860"/>
  </a>
</p>
