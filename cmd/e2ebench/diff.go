...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "wenhao.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 Wenhao e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "wenhao/internal/agent/foo.go" into
