# Wenhao Desktop (Wails shell)

A native desktop window around the Wenhao Go kernel. The same
...
`desktop/` is its own Go module (`module wenhao/desktop`, `replace wenhao =>
...
`wenhao/` so it can still import the `wenhao/internal/*` kernel.
...
```sh
cd desktop
wails build          # → build/bin/Wenhao(.app/.exe)
```
...
- **macOS** — open `Wenhao-darwin-universal.dmg` and drag Wenhao into
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/Wenhao.app
  ```
...
```sh
minisign -Vm Wenhao-darwin-arm64.zip \
