...
func TestSettingsUsesUserDesktopPreferencesNotProjectConfig(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "wenhao.toml"), []byte(`
[desktop]
language = "zh"
theme = "light"
theme_style = "glacier"
...
func TestSubmitToTabHistoryDisplaysRawInputAfterMemoryCompose(t *testing.T) {
	...
	ctrl.QueueMemory(`Saved memory "wenhao-contributions": contribution count updated`)
...
func TestForkCreatesActiveTabWithoutSwitchingSourceController(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(workspace, "wenhao.toml"), []byte("[codegraph]\nenabled = false\n"), 0o644); err != nil {
...
func TestCapabilitiesShowsDefaultMCPAsInitializingNotDisabled(t *testing.T) {
	...
	dir := robustTempDir(t)
	t.Chdir(dir)
	if err := os.WriteFile(filepath.Join(dir, "wenhao.toml"), []byte(`
[codegraph]
enabled = false
...
`), 0o644); err != nil {
	...
	projectCfg := config.LoadForEdit(filepath.Join(dir, "wenhao.toml"))
...
`), 0o644); err != nil {
	...
	if err := app.UpdateMCPServer("broken", MCPServerInput{
		Name:      "broken",
		Transport: "stdio",
		Command:   "wenhao-missing-mcp-binary",
	}); err != nil {
	...
	if got := cfg.Plugins[0].Command; got != "wenhao-missing-mcp-binary" {
	...
	for _, s := range view.Servers {
		if s.Name == "broken" {
			...
			if s.Command != "wenhao-missing-mcp-binary" || s.Tier != "background" {
...
func TestSetMCPServerTierPersistsCodegraphConfig(t *testing.T) {
	...
	t.Setenv("WENHAO_CACHE_DIR", robustTempDir(t)) // isolate the codegraph bundle cache so Resolve fails deterministically
	dir := robustTempDir(t)
	t.Chdir(dir)
	if err := os.WriteFile(filepath.Join(dir, "wenhao.toml"), []byte(`
[codegraph]
