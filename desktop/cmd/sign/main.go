...
// by substring (file names embed the key, e.g. Wenhao-darwin-arm64.zip), so the
...
// (wenhao.key) and the public key (wenhao.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "wenhao.key")
	pubPath := filepath.Join(dir, "wenhao.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "Wenhao desktop release")
		out := f + ".minisig"
