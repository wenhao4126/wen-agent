...
import (
	...
	"wenhao/desktop/internal/update"
)
...
func TestSignFiles(t *testing.T) {
	...
	dir := t.TempDir()
	artifact := filepath.Join(dir, "Wenhao-linux-amd64.tar.gz")
...
func TestGenManifest(t *testing.T) {
	dir := t.TempDir()
	names := []string{
		"Wenhao-darwin-arm64.zip",
		"Wenhao-darwin-amd64.zip",
		"Wenhao-windows-amd64-installer.exe",
		"Wenhao-windows-amd64.zip", // portable download, not the updater channel
		"Wenhao-linux-amd64.tar.gz",
		"Wenhao-linux-amd64.tar.gz.minisig", // must be skipped
	...
	t.Setenv("GITHUB_REPOSITORY", "esengine/wenhao")
	...
	wantURL := "https://github.com/esengine/wenhao/releases/download/desktop-v1.2.0/Wenhao-windows-amd64-installer.exe"
	if win.URL != wantURL {
