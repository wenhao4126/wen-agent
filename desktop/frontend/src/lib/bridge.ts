...
function makeMockApp(): AppBindings {
  ...
  const capSkills: SkillView[] = [
    ...
    { name: "init", description: "Scaffold a WENHAO.md for this repo", scope: "builtin", runAs: "inline", enabled: true },
  ];
  let capSkillRoots: SkillRootView[] = [
    { dir: "~/projects/wenhao/.wenhao/skills", scope: "project", priority: 1, status: "missing", configured: false, skills: 0 },
    {
      dir: "~/my-skills",
      scope: "custom",
      priority: 5,
    ...
    },
    {
      ...
      skillItems: [
        ...
        { name: "init", description: "Scaffold a WENHAO.md for this repo", scope: "global", runAs: "inline" },
      ],
    },
  ];
  ...
  const settings: SettingsView = {
    ...
    agent: { temperature: 0.2, maxSteps: 0, systemPrompt: "You are Wenhao, a coding agent." },
    desktopLanguage: "",
    desktopTheme: "dark",
    desktopThemeStyle: "graphite",
    closeBehavior: "background",
    configPath: "~/projects/wenhao/wenhao.toml",
  ...
  if (freshMock) {
    settings.configPath = "~/.config/wenhao/config.toml";
  }
  ...
  return {
    ...
    async PickWorkspace() {
      ...
      return mockSwitchWorkspace(cwd.endsWith("another-project") ? "~/projects/wenhao" : "~/projects/another-project");
    },
    ...
    async SavePastedImage(_dataUrl: string) {
      return ".wenhao/attachments/mock.png";
    },
    async SavePastedFile(name: string, _dataUrl: string) {
      return `.wenhao/attachments/mock-${name}`;
    },
    async AttachDropped(path: string) {
      const name = path.split(/[/\\]/).filter(Boolean).pop() ?? path;
      return { kind: "attachment" as const, path: `.wenhao/attachments/mock-${name}` };
    },
    ...
    async Memory() {
      return {
        available: true,
        storeDir: "~/.config/wenhao/projects/-mock/memory",
        docs: [
          {
            path: "WENHAO.md",
            scope: "project",
            body: "# Wenhao project memory\n\nMock doc shown in the browser dev seam.\n\n## Notes\n\n- prefers concise replies",
          },
          {
            path: "~/.config/wenhao/WENHAO.md",
            scope: "user",
        ...
        scopes: [
          { scope: "user", path: "~/.config/wenhao/WENHAO.md" },
          { scope: "project", path: "WENHAO.md" },
          { scope: "local", path: "WENHAO.local.md" },
        ],
      };
    },
    async Remember(scope: string, note: string) {
      emit({ kind: "notice", level: "info", text: `remembered → ${scope}` });
      return `${scope} WENHAO.md (mock): ${note}`;
    },
    ...
    async OpenDownloadPage() {
      if (typeof window !== "undefined") {
        window.open("https://github.com/esengine/wenhao/releases/latest", "_blank", "noopener");
      }
    },
    ...
    async ContextPanel(_tabID: string) {
      ...
      return {
        ...
        readFiles: [
          { path: "WENHAO.md", turn: 2, time: now - 34 * 60 * 1000 },
