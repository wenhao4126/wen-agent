...
const (
	...
	manifestFallback    = "https://github.com/esengine/wenhao/releases/latest/download/latest.json"
	defaultDownloadPage = "https://github.com/esengine/wenhao/releases/latest"
...
func applyLinux(targz []byte) error {
	bin, err := extractBinary(targz, "wenhao-desktop")
	if err != nil {
		return err
	}
...
func applyWindows(installer []byte) error {
	f, err := os.CreateTemp("", "wenhao-update-*.exe")
	if err != nil {
		return err
	}
	name := f.Name()
