# Migrating to Wenhao 1.0 (the Go rewrite)

Wenhao 1.0 is a **ground-up rewrite in Go**. It is a new codebase, not an
incremental upgrade of the `0.x` TypeScript releases. This guide explains what
changed and how to move over.

## TL;DR

| | Legacy (v1) | Wenhao 1.0+ (v2) |
|---|---|---|
| Language | TypeScript / Node | Go |
| Branch | [`v1`](https://github.com/esengine/DeepSeek-Wenhao/tree/v1) (maintenance only) | `main-v2` (default, active) |
| Versions | `0.x` (up to v0.54.x) | `1.0.0`+ |
| Install | `npm i -g wenhao` (the `latest` tag, stays on `0.x`) | `npm i -g wenhao@next` — `latest` deliberately stays on `0.x`; or a release archive / `go build` |
...
**`npm i -g wenhao` deliberately still installs `0.x`.** A bare install — and
`npx wenhao`, and 0.53's own `update` — follows npm's `latest` tag, which we
...
```sh
npm i -g wenhao@next     # or pin a version: wenhao@1.1.0
wenhao chat
```
...
Prebuilt archives (`wenhao-<os>-<arch>.tar.gz` / `.zip`) and the desktop
...
```sh
git clone https://github.com/esengine/DeepSeek-Wenhao   # default: main-v2 (Go)
cd DeepSeek-Wenhao && make build                        # -> bin/wenhao(.exe)
```
...
| Legacy | Wenhao 1.0 |
|---|---|
| TS config files | `wenhao.toml` (project) / `~/.config/wenhao/config.toml` (user) — see `wenhao.example.toml` |
| env / API keys | `.env` or the environment (`DEEPSEEK_API_KEY`, `MIMO_API_KEY`, …) via `api_key_env` |
| project memory | `WENHAO.md` (+ auto-memory), Claude-Code-compatible |
| MCP servers | `[[plugins]]` in `wenhao.toml`, or a Claude-Code `.mcp.json` (read as-is) |

On first launch v2 runs a one-time, **non-destructive** import: it reads a v0.x
`~/.wenhao/config.json` (API key, base URL, language, MCP servers) and imports
past sessions from `~/.wenhao/sessions` and legacy event logs already located
...
Wenhao 1.0 supports reading and editing files in UTF-8, UTF-8 BOM, UTF-16
...
Questions? Open a [Discussion](https://github.com/esengine/DeepSeek-Wenhao/discussions).
