# Wenhao Engineering Spec

> Wenhao is a coding agent: a thin harness driving multiple models, with **all
...
```
wenhao/
├── go.mod / go.sum          # module wenhao; require BurntSushi/toml
├── Makefile                 # build / cross / vet / fmt / test
├── README.md / README.zh-CN.md
├── wenhao.example.toml         # sample config
├── docs/SPEC.md             # this file
├── cmd/wenhao/main.go          # entry; blank-imports built-in providers/tools
├── cmd/wenhao-plugin-example/  # reference MCP stdio plugin (a runnable example)
└── internal/
    ...
    ├── command/             # custom slash commands loaded from .wenhao/commands/*.md
...
- `cmd/wenhao-plugin-example` is a runnable reference stdio server (`echo`,
...
Long tasks eventually fill the model's context window. Wenhao manages this with
...
- Compaction summarizes the older middle of the session into a single briefing —
  ...
  summarized away.
- The dropped originals are archived to `~/.config/wenhao/archive/<timestamp>.jsonl`
...
- **Resolving `Ask`.** The interactive front-end (the chat TUI) prompts the user
  ...
  (`wenhao run`, a sub-agent, anything with no TTY / no approver) cannot prompt, so
...
Out of the box (`mode = "ask"`, no rules) `wenhao run` behaves exactly as before
(writers resolve `Ask`→allow with no TTY), while `wenhao chat` now prompts before
...
- **Custom commands** are Markdown files under `.wenhao/commands/` (project) and
  `~/.config/wenhao/commands/` (user); the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — Wenhao
...
Resolution order: **flag > project `./wenhao.toml` > user `~/.config/wenhao/config.toml`
...
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $WENHAO_LANG

[agent]
system_prompt = "You are Wenhao, a coding agent..."  # or system_prompt_file = "..."
max_steps     = 25
temperature   = 0.0
...
`wenhao setup` writes this default config so the CLI is usable out of the box.
...
`[[plugins]]`; on a name collision `wenhao.toml` wins (it is the more explicit,
Wenhao-specific source). This lets a server already configured for Claude work in
Wenhao unchanged.
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o wenhao ./cmd/wenhao`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  command unconfined via the permission gate (in `wenhao run`, the command just
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `wenhao run`.
