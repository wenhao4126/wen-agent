...
type Agent struct {
	...
	// sessionMemInjector, when non-nil, returns session-memory context to
	// fold into the compaction summary prompt for richer summaries.
	sessionMemInjector func() string

	// recentFiles tracks recently-read files for post-compact restoration.
...
// SetGate installs the per-call permission gate. Used by `wenhao chat` to swap the
...
// `wenhao chat --resume` to load a saved JSONL transcript before the first turn,
...
func (a *Agent) executeOne(ctx context.Context, call provider.ToolCall) toolOutcome {
	...
	// Track read_file results for post-compact file restoration.
	if call.Name == "read_file" && err == nil {
		var rp struct {
			FilePath string `json:"file_path"`
		}
		if json.Unmarshal([]byte(call.Arguments), &rp) == nil && rp.FilePath != "" {
			a.TrackReadFile(rp.FilePath, result)
		}
	}
