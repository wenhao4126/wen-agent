...
const systemPrompt = "You are wenhao, a coding agent. Be concise and follow project conventions. " +
...
func TestReleaseCacheHitGuard(t *testing.T) {
	if os.Getenv("WENHAO_RELEASE_CACHE_GUARD") == "" {
		t.Skip("set WENHAO_RELEASE_CACHE_GUARD=1 to run the release cache guard")
	}

	threshold := envInt("WENHAO_CACHE_GUARD_THRESHOLD", 90)
	maxLowCases := envInt("WENHAO_CACHE_GUARD_MAX_LOW_CASES", 1)
	...
	if len(lows) > maxLowCases {
		...
		if os.Getenv("WENHAO_CACHE_GUARD_STRICT") != "" {
			t.Fatal(msg)
		}
	}
}
