...
// importantMarker is a prefix the model can prepend to a message to mark it as
// worth preserving verbatim during compaction. The marker is stripped before
// sending to the provider and only acts as a compaction hint.
const importantMarker = "<!--IMPORTANT-->"

// compact summarizes the older middle of the session and replaces it in place.
// Layered compaction: if prior compaction summaries exist, only the newest
// uncompacted segment is summarized — older summaries are preserved as-is.
// Messages marked with <!--IMPORTANT--> are kept verbatim rather than summarized.
// Post-compaction, recently-read files may be re-injected as context blocks.
func (a *Agent) compact(ctx context.Context, trigger, instructions string, force bool) error {
	msgs := a.session.Messages

	// Layered compaction: find the boundary between already-compacted and new
	// messages. Already-compacted messages are ones inside <compaction-summary>
	// blocks and the summaries themselves. Only compact the newest segment.
	head, start, ok := a.planLayeredCompaction(msgs, minCompactMessages)
	if !ok {
		head, start, ok = a.planLayeredCompaction(msgs, 1)
	}
	if !ok {
		return nil
	}
	...
	// Separate important messages from the compactable region.
	compactable, important := partitionImportant(region)
	if len(compactable) == 0 {
		return nil // everything is marked important
	}

	// Economic check on the compactable portion.
	if !force && !foldEconomics(compactable) {
		return nil
	}

	a.sink.Emit(event.Event{Kind: event.CompactionStarted, Compaction: event.Compaction{Trigger: trigger}})

	// PreCompact hook + session memory injection.
	if a.hooks != nil {
	...
	// Inject session memory into the summary prompt for richer compaction.
	if a.sessionMemInjector != nil {
		if smInj := a.sessionMemInjector(); smInj != "" {
			if instructions != "" {
				instructions += "\n"
			}
			instructions += smInj
	...
	archived := ""
	if a.archiveDir != "" {
		path, err := archiveMessages(a.archiveDir, compactable)
		if err != nil {
	...
	summary, err := a.summarize(ctx, compactable, instructions)
	if err != nil {
		a.emitCompactionAborted(trigger)
		return err
	}
	...
	//   prefix (head) → important preserved messages → new compaction summary → tail
	compacted := make([]provider.Message, 0, head+len(important)+1+len(msgs)-start)
	...
	// Post-compact: re-inject recently-read file contents so the model doesn't
	// "forget" what it was working on after compaction removed the read_file results.
	a.restoreRecentFiles(ctx)
...
// planLayeredCompaction finds the compaction boundary. Unlike planCompaction
// which always starts from head (after system prompt), this finds the first
// message after the last compaction summary block, so already-summarized
// messages are preserved and only the newest segment is compacted.
func (a *Agent) planLayeredCompaction(msgs []provider.Message, min int) (head, start int, ok bool) {
	head = 0
	if len(msgs) > 0 && msgs[0].Role == provider.RoleSystem {
		head = 1
	}

	// Find the last compaction summary block — messages before it (plus the
	// summary itself) are already compacted and should be preserved.
	lastCompactionEnd := head
	for i := head; i < len(msgs); i++ {
		if msgs[i].Role == provider.RoleUser && strings.Contains(msgs[i].Content, summaryTagOpen) {
			// Found a compaction summary. The compacted region ends after this
			// message. The NEW head for the next compaction starts here.
			head = i + 1
			lastCompactionEnd = head
		}
	}

	// Now plan the tail from the new head.
...
// partitionImportant splits a message region into compactable and important
// messages. Important messages start with <!--IMPORTANT--> and are preserved
// verbatim during compaction.
func partitionImportant(msgs []provider.Message) (compactable, important []provider.Message) {
	for _, m := range msgs {
		if strings.HasPrefix(strings.TrimSpace(m.Content), importantMarker) {
...
// SetSessionMemoryInjector installs a callback that returns session-memory
// context to inject into the compaction summary prompt. Set from boot.
func (a *Agent) SetSessionMemoryInjector(fn func() string) {
	a.sessionMemInjector = fn
}

// recentFiles tracks files the agent has read, so post-compact restoration
// can re-inject key files the model was working with.
type recentFile struct {
	path    string
	content string // truncated if too large
}

const maxRecentFiles = 5
const maxRecentFileTokens = 5000

// TrackReadFile records that a file was read, storing a truncated version for
// post-compact restoration. Called from the read_file tool's execute path.
func (a *Agent) TrackReadFile(path, content string) {
	a.recentFilesMu.Lock()
	defer a.recentFilesMu.Unlock()

	// Truncate to keep the stored content bounded.
	if len(content) > maxRecentFileTokens*4 {
		content = content[:maxRecentFileTokens*4] + "\n... (truncated)"
	}

	// Update or append.
	for i, rf := range a.recentFiles {
		if rf.path == path {
			a.recentFiles[i].content = content
			// Move to end (most recent).
			a.recentFiles = append(a.recentFiles[:i], a.recentFiles[i+1:]...)
			a.recentFiles = append(a.recentFiles, recentFile{path: path, content: content})
			return
		}
	}
	...
	if len(a.recentFiles) > maxRecentFiles {
		a.recentFiles = a.recentFiles[1:]
	}
}

// restoreRecentFiles re-injects recently-read file contents as system-context
// messages after a compaction, so the model doesn't lose visibility of files
// it was actively working on.
func (a *Agent) restoreRecentFiles(ctx context.Context) {
	a.recentFilesMu.Lock()
	files := make([]recentFile, len(a.recentFiles))
	copy(files, a.recentFiles)
	...
	b.WriteString("## Recently-read files (restored after compaction)\n\n")
	b.WriteString("The following files were read before compaction. Their contents are re-injected so you can continue working without re-reading them:\n\n")
