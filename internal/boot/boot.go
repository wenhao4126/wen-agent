...
import (
	...
	"wenhao/internal/agent"
	"wenhao/internal/codegraph"
	"wenhao/internal/command"
	"wenhao/internal/config"
	"wenhao/internal/control"
	"wenhao/internal/event"
	"wenhao/internal/hook"
	"wenhao/internal/instruction"
	"wenhao/internal/jobs"
	"wenhao/internal/lsp"
	"wenhao/internal/memory"
	"wenhao/internal/netclient"
	"wenhao/internal/outputstyle"
	"wenhao/internal/permission"
	"wenhao/internal/plugin"
	"wenhao/internal/provider"
	"wenhao/internal/sandbox"
	"wenhao/internal/skill"
	"wenhao/internal/tool"
	"wenhao/internal/tool/builtin"
)
...
type Options struct {
	...
	// controller but are not persisted to wenhao.toml.
...
func Build(ctx context.Context, opts Options) (*control.Controller, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w %q (configured: %s); note: defining [[providers]] replaces the built-in presets, so add a [[providers]] entry for it or use a configured name, or run `wenhao setup` to reconfigure", ErrUnknownModel, modelName, providerNames(cfg))
	}
	...
	if migErr != nil {
		sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: "config migration from ~/.wenhao failed: " + migErr.Error()})
	...
	// Persistent memory (WENHAO.md / AGENTS.md hierarchy + auto-memory index)
	...
	if cfg.Codegraph.Enabled {
		...
		default:
			sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelInfo,
				Text: "codegraph: not installed — run `wenhao codegraph install` to enable symbol-graph tools"})
		}
	}
	...
	// resolves "ask" to allow — preserving `wenhao run` autonomy — while deny
	...
	// Agent registry: load built-in agent definitions, then overlay any
	// user-configured [[agents]] entries from wenhao.toml.
	agentDefs := agent.BuiltinAgentDefs()
	for _, a := range cfg.Agents {
		agentDefs = append(agentDefs, agent.AgentDefinition{
			Type:            a.Type,
			...
			Model:           a.Model,
			Effort:          a.Effort,
			Isolation:       agent.IsolationMode(a.Isolation),
			MaxSteps:        a.MaxSteps,
		})
	}
	agentReg := agent.NewRegistry(agentDefs)

	// AgentTool: the unified sub-agent spawning tool. Deferred to after
	// executor session creation so fork mode can access parent messages.

	// Agent listing attachment: inject available agent types into the system
	// prompt so the model knows which specialized agents it can spawn.
	if listing := agentReg.AgentListingAttachment(); listing != "" {
		sysPrompt += "\n\n" + listing
	}
	...
	// Context management tools: diff_preview for verifying SEARCH/REPLACE
	// edits before applying (replace_context is registered after executor
	// session creation since it requires the session reference).
	reg.Add(agent.NewDiffPreviewTool(root))
	...
	// Session memory injector is wired after sessionMem is created below.

	// Custom slash commands (.wenhao/commands + user dir). Best-effort: a malformed
	...
	// Session memory: a per-session memory.md that the agent updates incrementally in
	// the background. Lives under the session directory. Mirrors Claude Code's
	// SessionMemory design: token-growth-triggered extractions via forked sub-agent.
	sessionMemDir := filepath.Join(config.SessionDir(), "memory")
	sessionMem := memory.NewSessionMemory(sessionMemDir, memory.DefaultSessionMemoryConfig)
	if err := sessionMem.EnsureFile(); err != nil {
		sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: "session memory file unavailable: " + err.Error()})
	}

	// Wire session memory injection into compaction summaries.
	executor.SetSessionMemoryInjector(func() string {
		return sessionMem.Injection()
	})

	// Durable memory extraction: automatically saves reusable knowledge from
	// conversations to the project's auto-memory store.
	extractMem := memory.NewExtractMemories(mem.Store, memory.DefaultExtractMemoriesConfig)

	// Post-turn runner: fires after each turn to drive background memory work.
	postTurn := agent.NewPostTurnRunner(agent.PostTurnConfig{
		SessionMemory:  newBootSessionMemoryDriver(sessionMem, execProv, sysPrompt),
		ExtractMemories: newBootExtractMemoriesDriver(extractMem, mem.Store, execProv, sysPrompt),
	}, sink, execProv, sysPrompt)

	// Dream: cross-session memory consolidation. Periodically reviews recent
	// session memory files and the durable memory store, then merges/updates/
	// removes memories to keep the store high-signal. Fires on session startup
	// when the time gate (24h) and session-count gate (5 sessions) both pass.
	dreamCfg := memory.DefaultDreamConfig(
		config.SessionDir(),        // session memory files live here
		filepath.Join(config.MemoryUserDir(), ".."), // dream state in ~/.wenhao/
	)
	dream := memory.NewDream(dreamCfg, mem.Store, execProv, sysPrompt, sink)
...
func migrateLegacySessionSources(sink event.Sink) {
	...
	if home, herr := os.UserHomeDir(); herr == nil {
		sources = append(sources, legacySource{
			dir:     filepath.Join(home, ".wenhao", "sessions"),
			label:   "~/.wenhao/sessions",
...
func rememberPermissionConfigPath(workspaceRoot string) string {
	...
	if workspaceRoot != "" {
		return filepath.Join(workspaceRoot, "wenhao.toml")
	}
	path := config.SourcePath()
	if path == "" {
		path = "wenhao.toml" // match Config.Save() fallback
	}
	return path
}
...
// --- Memory driver adapters ---
// These bridge the memory package's types to the agent package's driver
// interfaces, so boot can wire them without the agent depending on memory.

type bootSessionMemoryDriver struct {
	sm   *memory.SessionMemory
	prov provider.Provider
	sys  string
}

func newBootSessionMemoryDriver(sm *memory.SessionMemory, prov provider.Provider, sys string) *bootSessionMemoryDriver {
	return &bootSessionMemoryDriver{sm: sm, prov: prov, sys: sys}
}

func (d *bootSessionMemoryDriver) ShouldExtract(msgs []provider.Message) bool {
	return d.sm.ShouldExtract(msgs)
}

func (d *bootSessionMemoryDriver) StartExtraction(msgs []provider.Message) bool {
	return d.sm.MarkExtractionStarting(msgs)
}

func (d *bootSessionMemoryDriver) FinishExtraction() []provider.Message {
	return d.sm.MarkExtractionDone()
}

func (d *bootSessionMemoryDriver) Run(ctx context.Context, msgs []provider.Message, prov provider.Provider, sysPrompt string) (string, error) {
	current := d.sm.Content()
	// Only send the last ~40 messages to keep the extraction prompt bounded.
	transcript := memory.RecentTranscript(msgs, 40)
	if transcript == "" {
		return "", nil
	}
	prompt := memory.BuildSessionMemoryPrompt(current, transcript)
	...
	if err != nil {
		return "", fmt.Errorf("session memory stream: %w", err)
	}
	...
	updated := strings.TrimSpace(result.String())
	if updated == "" {
		return "", nil
	}

	if err := os.WriteFile(d.sm.Path(), []byte(updated), 0o644); err != nil {
		return "", fmt.Errorf("write session memory: %w", err)
	}
	return "session memory updated", nil
}

func (d *bootSessionMemoryDriver) Content() string {
	return d.sm.Content()
}

type bootExtractMemoriesDriver struct {
	em    *memory.ExtractMemories
	store memory.Store
	prov  provider.Provider
	sys   string
}

func newBootExtractMemoriesDriver(em *memory.ExtractMemories, store memory.Store, prov provider.Provider, sys string) *bootExtractMemoriesDriver {
	return &bootExtractMemoriesDriver{em: em, store: store, prov: prov, sys: sys}
}

func (d *bootExtractMemoriesDriver) ShouldExtract(msgs []provider.Message) bool {
	return d.em.ShouldExtract(msgs)
}

func (d *bootExtractMemoriesDriver) StartExtraction(msgs []provider.Message) bool {
	return d.em.MarkExtractionStarting(msgs)
}

func (d *bootExtractMemoriesDriver) FinishExtraction() []provider.Message {
	return d.em.MarkExtractionDone(nil)
}

func (d *bootExtractMemoriesDriver) Run(ctx context.Context, msgs []provider.Message, prov provider.Provider, sysPrompt string) (string, error) {
	newCount := len(msgs)
	manifest := memory.FormatMemoryManifest(d.store)
	prompt := memory.BuildExtractMemoriesPrompt(newCount, manifest)
	...
	if err != nil {
		return "", fmt.Errorf("extract memories stream: %w", err)
	}
	...
	_ = strings.TrimSpace(result.String())
	// For now, durable memory extraction is a stub — it runs the prompt but
	// doesn't write to the store. A full implementation would fork a sub-agent
	// with tool access to the memory directory (see Phase 2).
	return "", nil
}
