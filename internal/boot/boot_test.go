...
import (
	...
	// Blank import registers the provider kind the same way cmd/wenhao's main
...
// cache-first wiring: a project WENHAO.md is discovered at boot and folded
...
model = "x"
api_key_env = "WENHAO_TEST_KEY_UNSET"
`)
	writeFile(t, dir, "WENHAO.md", "Project rule: always run go vet before committing.")
	...
	if !strings.Contains(sys, "always run go vet before committing") {
		t.Fatalf("project WENHAO.md not folded into system message:\n%s", sys)
	}
	...
	if mem := ctrl.Memory(); mem == nil || len(mem.Docs) == 0 {
		t.Fatal("controller memory set is empty after discovering WENHAO.md")
	}
}
...
api_key_env = "WENHAO_TEST_KEY_UNSET"
`)
	writeFile(t, dir, ".wenhao/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
`)
	...
	// user-global WENHAO.md in the real config dir could append; the test
...
func TestRememberPermissionRuleUsesWorkspaceRoot(t *testing.T) {
	...
	t.Chdir(cwd)
	writeFile(t, cwd, "wenhao.toml", `
[permissions]
allow = ["bash(cwd*)"]
`)
	writeFile(t, workspace, "wenhao.toml", `
[permissions]
...
`)
	...
	cwdCfg := config.LoadForEdit(filepath.Join(cwd, "wenhao.toml"))
	...
	workspaceCfg := config.LoadForEdit(filepath.Join(workspace, "wenhao.toml"))
...
`)
	...
	if _, err := os.Stat(filepath.Join(cwd, "wenhao.toml")); !os.IsNotExist(err) {
...
// ~/.wenhao/config.json with no v1+ config present must be imported during
...
func TestBuildMigratesLegacyConfigEndToEnd(t *testing.T) {
	...
	writeFile(t, proj, "wenhao.toml", "[codegraph]\nenabled = false\n")
	writeFile(t, filepath.Join(home, ".wenhao"), "config.json",
		`{"apiKey":"sk-e2e","lang":"zh","mcpServers":{"fs":{"command":"npx","args":["-y","server-fs"]}}}`)
	writeFile(t, filepath.Join(home, ".wenhao", "sessions"), "chat-1.events.jsonl",
...
[[plugins]]
name = "legacy-eager"
command = "wenhao-missing-legacy-eager-mcp"
tier = "eager"
`)
	...
	raw, err := os.ReadFile(filepath.Join(dir, "wenhao.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
[[plugins]]
name = "legacy-lazy"
command = "wenhao-missing-legacy-lazy-mcp"
tier = "lazy"
`)
...
func TestBuildColdCodegraphStartsInBackground(t *testing.T) {
	...
	writeFile(t, dir, "wenhao.toml", fmt.Sprintf(`
default_model = "test-model"
...
[[plugins]]
name = "slowserver"
command = "wenhao-missing-slow-mcp-binary"
tier = "eager"
`)
