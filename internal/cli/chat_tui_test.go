...
func TestMainManagerFollowsTranscriptWithoutTopPadding(t *testing.T) {
	...
	m = m0.(chatTUI)
	m.wrappedLines = []string{"wenhao chat", "› /mcp"}
	...
	if !strings.Contains(lines[0], "wenhao chat") || !strings.Contains(lines[1], "/mcp") {
...
func TestAutoPlanCommandWritesUserConfigNotProjectConfig(t *testing.T) {
	isolateUserConfig(t)
	projectPath := filepath.Join(mustGetwd(t), "wenhao.toml")
...
func TestLanguageCommandAutoClearsLowerPriorityUserOverride(t *testing.T) {
	...
	if err := projectCfg.SaveTo("wenhao.toml"); err != nil {
