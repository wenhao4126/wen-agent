// Package cli implements wenhao's command-line entry: subcommand routing, flag
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `wenhao init` isn't a dead end.
		configureCLIThemeFromConfigNoProbe()
		return initHint()
	case "acp":
		configureCLIThemeFromConfigNoProbe()
...
func runServe(args []string) int {
	...
	fmt.Printf("wenhao serve — %s on http://%s\n", ctrl.Label(), *addr)
...
// Keys always go to the wenhao-owned global credentials file so they never land
...
// project-local wenhao.toml only when the user config dir can't be resolved.
...
// defaultEnvTarget is the wenhao-owned global credentials file, falling back to
...
// resolveSetupTargets picks where `wenhao setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./wenhao.toml
...
// setupConfig runs the configuration wizard (the `wenhao setup` command),
// writing config.toml to the user-global dir (or ./wenhao.toml under --local)
// and API keys to the wenhao-owned global .env — never a project's own .env.
...
// initHint handles `wenhao init`. Unlike a config scaffold, project memory is
...
// (and to `wenhao setup` for config) so the verb isn't a dead end.
func initHint() int {
...
// and any entered API keys to envPath (the wenhao-owned global .env, never a
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("wenhao")),
		"",
...
// that older versions of the wizard wrote into wenhao.toml. They collide
...
// can warn the user to clean up wenhao.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `wenhao setup`.
// The host-based slug also gives users a meaningful name to grep for in
// wenhao.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing wenhao.toml deal with SKU names
...
// wenhao.toml that defines only [[providers]] for deepseek otherwise hides
...
// wenhao-owned global .env so the chat session that follows picks them up via
...
// are appended — so re-running `wenhao setup` with a corrected key replaces the
...
func welcome(version string) int {
	...
	b.WriteString(boxed([]string{
		accent("◆") + " " + bold("wenhao") + "  " + dim(version),
		dim(i18n.M.Subtitle),
	}))
	...
	if src == "" {
		step("wenhao setup", i18n.M.StepScaffold)
	}
	if ready == 0 {
		step(i18n.M.StepSetKey, i18n.M.StepSetKeyHint)
	}
	step("wenhao chat", i18n.M.StepChatDesc)
	step(`wenhao run "task"`, i18n.M.StepRunDesc)
...
func configAutoPlanCommand(args []string) int {
	...
	local := fs.Bool("local", false, "write ./wenhao.toml instead of the user config")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	rest := fs.Args()
...
func configUsage() {
	fmt.Print(`Usage:
  wenhao config auto-plan [--local] [off|on]
`)
}
