...
// codegraphCommand backs `wenhao codegraph` — managing the CodeGraph
// code-intelligence runtime that wenhao otherwise fetches lazily on first use.
...
func codegraphStatus() int {
	...
	if p, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-13s %s\n", "resolved:", "(not installed — run `wenhao codegraph install`)")
	}
	return 0
}

func codegraphUsage() {
	fmt.Print(`wenhao codegraph — manage the CodeGraph code-intelligence runtime

Usage:
  wenhao codegraph install   download + cache the runtime for this platform
  wenhao codegraph status    show config, cache dir, and resolved launcher
