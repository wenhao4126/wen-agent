...
func renderHooks(width int, hooks []hook.ResolvedHook, trusted bool, projectDefines bool) string {
	...
	case trusted:
		b.WriteString(viewHint(viewCompactText("project trusted · config: project .wenhao/settings.json + global ~/.wenhao/settings.json", viewBudget(width, 2))))
	default:
		b.WriteString(viewHint(viewCompactText("project not trusted · config: project .wenhao/settings.json + global ~/.wenhao/settings.json", viewBudget(width, 2))))
	}
