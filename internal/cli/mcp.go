...
// mcp.go holds the MCP server-management surface shared by the `wenhao mcp`
...
// mcpCommand implements `wenhao mcp <add|remove|list>`. It edits config only
...
func mcpList() int {
	...
	if bin, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-16s (built-in, not installed)%s  run `wenhao codegraph install`", "codegraph", codegraphMeta)
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: wenhao mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to wenhao.toml).

Usage:
  wenhao mcp list
  wenhao mcp add <name> <command> [args...]        stdio server
  wenhao mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  wenhao mcp add <name> --sse  <url>               remote (legacy SSE)
  wenhao mcp remove <name>
...
Examples:
  wenhao mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  wenhao mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
