...
func configureCLIThemeWithStyle(mode, style string) {
	if env := strings.TrimSpace(os.Getenv("WENHAO_THEME")); env != "" {
	...
	if env := strings.TrimSpace(os.Getenv("WENHAO_THEME_STYLE")); env != "" {
		style = env
	}
...
func refreshCLIStyles() {
	...
	todoPanelStyle = withThemeBorderFG(lipgloss.NewStyle().
		...
		PaddingLeft(1)
	agentPanelStyle = withThemeBorderFG(lipgloss.NewStyle().
		Border(lipgloss.NormalBorder(), true, false, false, false), activeCLITheme.accent).
		PaddingLeft(1)
