...
// canonical .wenhao project dir is highest priority (last, since command.Load
...
func TestCommandDirsIncludeConventions(t *testing.T) {
	...
	for _, want := range []string{
		...
		filepath.Join(".wenhao", "commands"),
	} {
		if !strings.Contains(joined, want) {
			t.Errorf("CommandDirs missing %q\ngot:\n%s", want, joined)
		}
	}
	// The project's .wenhao/commands must be the highest-priority (last) entry.
	if last := dirs[len(dirs)-1]; last != filepath.Join(".wenhao", "commands") {
		t.Errorf("project .wenhao/commands should be highest priority (last), got %q", last)
	}
}
