// Package config loads Wenhao's runtime configuration from TOML. Resolution order:
// flag > project ./wenhao.toml > user ~/.config/wenhao/config.toml > built-in defaults.
...
// Config is Wenhao's runtime configuration.
type Config struct {
	ConfigVersion int                 `toml:"config_version"`
	DefaultModel  string              `toml:"default_model"`
	Language      string              `toml:"language"` // ui/model language tag (e.g. "zh"); empty = auto-detect from $LANG / $WENHAO_LANG
	UI            UIConfig            `toml:"ui"`
	...
	Agents        []AgentDefEntry     `toml:"agents"`
...
// lets wenhao fetch the CodeGraph runtime into its cache when CodeGraph is
// enabled but missing; set false to require an explicit `wenhao codegraph
...
// the project roots (.wenhao/.agents/.claude under the workspace) and the
...
type AgentConfig struct {
	...
	// .wenhao/output-styles/<name>.md). Empty = the unmodified prompt.
...
// wenhao.toml's [[plugins]] or a project-root .mcp.json (see loadMCPJSON).
...
const DefaultSystemPrompt = `You are Wenhao, a coding agent focused on executing code tasks.
...
// AgentDefEntry is a user-configured agent definition from wenhao.toml's
// [[agents]] section. It mirrors the agent.AgentDefinition fields; the config
// layer is independent of the agent package so boot maps between them.
...
func Default() *Config {
	return &Config{
		...
		// Mode "ask" with no rules keeps `wenhao run` autonomous (no TTY → ask
		// resolves to allow) while `wenhao chat` prompts before writers. Users add
...
// the v0.x ~/.wenhao/config.json's mcpServers. A .env in the working directory
...
// each project's wenhao.toml + .env + .mcp.json are resolved independently
...
func LoadForRoot(root string) (*Config, error) {
	...
	projectTOML := "wenhao.toml"
	if root != "." {
		projectTOML = filepath.Join(root, "wenhao.toml")
	}
	...
	// project wenhao.toml doesn't drop the global config's MCP servers.
	...
	// wenhao.toml wins on a name collision (see mergeMCPJSON).
	mcpFile := mcpJSONFile
	if root != "." {
	...
	// Lowest priority: the v0.x ~/.wenhao/config.json's mcpServers, so upgrading
...
// LoadForEdit returns a config to seed the `wenhao setup` wizard when reconfiguring:
...
func userConfigPath() string {
	...
	return filepath.Join(dir, "wenhao", "config.toml")
}

// UserConfigPath is the user-global config file (~/.config/wenhao/config.toml),
// or "" when the user config dir can't be resolved.
func UserConfigPath() string { return userConfigPath() }

// UserCredentialsPath is the wenhao-owned global secrets file, beside
// config.toml in the user config dir (e.g. ~/.config/wenhao/credentials). It
...
func UserCredentialsPath() string {
	...
	return filepath.Join(dir, "wenhao", "credentials")
}
...
func ArchiveDir() string {
	...
	return filepath.Join(dir, "wenhao", "archive")
}

// SessionDir is where chat sessions are persisted (one .jsonl per session).
// Used by `wenhao chat --continue` / `--resume` to find the recent ones. Empty
...
func SessionDir() string {
	...
	return filepath.Join(dir, "wenhao", "sessions")
}
...
// existing dirs (UserConfigDir/wenhao/...) so the whole wenhao state tree
...
func CacheDir() string {
	...
	return filepath.Join(dir, "wenhao", "cache")
}

// MemoryUserDir returns the wenhao user config root (…/wenhao), under which
// the user-global WENHAO.md and the per-project auto-memory store live. Empty
...
func MemoryUserDir() string {
	...
	return filepath.Join(dir, "wenhao")
}

// ConventionDirs are the parent directories scanned for agent assets (skills,
// commands), in canonical-first order. .wenhao is ours; .agents / .agent /
...
// .wenhao/settings.json (see internal/hook).
var ConventionDirs = []string{".wenhao", ".agents", ".agent", ".claude"}

// conventionSubdirsAsc joins sub under each ConventionDir of base, in ascending
// priority (reverse of ConventionDirs) so the canonical .wenhao ends up the
...
// on a name clash. Order: home-dir convention dirs (~/.claude/commands … ~/.wenhao/commands),
// the legacy XDG user dir (~/.config/wenhao/commands), then the project's
// convention dirs (.claude/commands … .wenhao/commands). Scanning the .claude /
...
func CommandDirsForRoot(root string) []string {
	...
	if dir, err := os.UserConfigDir(); err == nil {
		dirs = append(dirs, filepath.Join(dir, "wenhao", "commands")) // legacy XDG user dir
	}
