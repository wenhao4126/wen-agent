...
import (
	...
	"wenhao/internal/fileutil"
	"wenhao/internal/mcpdiag"
...
// separate from the `wenhao setup` wizard (cli) so a GUI can apply one setting at a
...
// SetLanguage pins the CLI UI/model language; empty/auto clears the override so runtime detection falls back to WENHAO_LANG / locale.
...
// .mcp.json-only server would otherwise be serialized into wenhao.toml or the
...
func pluginTOMLSourcePath(name string) string {
	for _, path := range []string{"wenhao.toml", userConfigPath()} {
...
// half-written wenhao.toml that fails to parse on next load. Parent directories
...
func SaveMinimalProjectAutoPlan(path, mode string) (string, error) {
	...
	body := fmt.Sprintf(`# Wenhao project configuration.
...
func writeConfigFile(path, body string) error {
	...
	tmp, err := os.CreateTemp(dir, ".wenhao.*.toml.tmp")
	if err != nil {
...
// (SourcePath), or to ./wenhao.toml when none exists yet — the conventional
...
// SaveForRoot saves the config to root's wenhao.toml, falling back to the
// user's global config when root has no existing wenhao.toml.
