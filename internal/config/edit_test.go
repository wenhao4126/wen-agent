...
func TestPluginMutators(t *testing.T) {
	...
	if err := c.UpsertPlugin(PluginEntry{Name: "ex", Command: "wenhao-plugin-example"}); err != nil {
...
func TestLoadForEditPreservesCodegraphWithoutSection(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "wenhao.toml")
...
func TestSaveToRoundTrips(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "nested", "wenhao.toml")
...
func TestSaveToScopesUserAndProjectFiles(t *testing.T) {
	...
	projectPath := filepath.Join(t.TempDir(), "wenhao.toml")
