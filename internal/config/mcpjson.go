...
// mcpJSONFile is the project-root file Claude Code calls .mcp.json. Wenhao reads
...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.wenhao/config.json.
func legacyConfigPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".wenhao", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.wenhao/config.json and returns its enabled
...
// already declare. wenhao.toml's [[plugins]] win on a name collision: it is the
// Wenhao-specific, more explicit of the two, so it overrides the shared,
