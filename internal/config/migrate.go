...
// legacyConfig is the subset of the v0.x (~/.wenhao/config.json) schema this
// import carries forward. Fields absent here are dropped on purpose: desktop tab
// state is frontend-owned, and skills already live in the shared ~/.wenhao/skills
...
func (r *MigrationResult) Notice() string {
	...
	if r.KeyToEnv {
		b.WriteString("; API key saved to wenhao's credentials store")
	}
...
// checks v1-era TOML first, then v0.5/v0.x ~/.wenhao/config.json, and never
...
func MigrateLegacyIfNeeded() (*MigrationResult, error) {
	...
	// Try legacy paths: .wenhao first (new brand), then .reasonix (old brand).
	src := filepath.Join(home, ".wenhao", "config.json")
...
func legacyTOMLPaths(dest, home string) []string {
	paths := []string{filepath.Join(filepath.Dir(dest), "wenhao.toml")}
	if home != "" {
		paths = append(paths, filepath.Join(home, ".wenhao", "wenhao.toml"))
	}
	return paths
}
...
// writeCredentialsEnv merges lines into the wenhao-owned global credentials
// file (UserCredentialsPath, e.g. %AppData%\wenhao\credentials), replacing any
