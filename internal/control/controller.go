...
import (
	...
	"wenhao/internal/billing"
	"wenhao/internal/checkpoint"
...
type Controller struct {
	...
	mem          *memory.Set
	postTurn     agent.PostTurner // post-turn memory extraction; nil when not wired
	dream        *memory.Dream    // cross-session memory consolidation
	cleanup      func()
	autoPlan     string
	...
	// sessionMem is the per-session memory manager (owns memory.md). nil when not
	// configured. Used by the controller for compaction injection.
...
type Options struct {
	...
	// PostTurn, when set, fires after every runner.Run completes to drive
	// background memory extraction (session memory + durable memory).
	PostTurn agent.PostTurner
	// SessionMemory is the per-session memory file manager; nil when not wired.
	SessionMemory *memory.SessionMemory
	// Dream, when set, drives cross-session memory consolidation. The controller
	// calls MaybeDream after session startup.
	Dream *memory.Dream
}
...
func (c *Controller) runTurnWithRawDisplay(ctx context.Context, input, raw, display string) error {
	...
	// Post-turn memory extraction: fire-and-forget background work that updates
	// session memory and extracts durable memories. Runs after every runner.Run
	// completion so even plan research turns contribute.
	c.afterTurn(ctx)
	c.mu.Lock()
	plan := c.planMode
	c.mu.Unlock()
	...
	// Post-turn after plan execution as well.
	c.afterTurn(ctx)
...
// afterTurn fires the post-turn hook (session memory + durable memory extraction)
// if wired. It's called after every runner.Run completes. The hook runs in a
// background goroutine so the plan-approval flow isn't blocked.
func (c *Controller) afterTurn(ctx context.Context) {
	if c.postTurn == nil {
		return
	}
	c.postTurn.AfterTurn(ctx, c.History())
}
...
// headless `wenhao run` path, where the Sink renders to stdout and the caller
...
// fires it too (its first post-resume turn). Also triggers Dream consolidation
// check (fire-and-forget, won't block the turn).
func (c *Controller) maybeSessionStart(ctx context.Context) {
	...
	// Dream: fire-and-forget cross-session memory consolidation check.
	if c.dream != nil {
		go c.dream.MaybeDream(ctx)
	}
}
...
// out of project wenhao.toml while preserving the CLI AddMCPServer semantics.
...
// WENHAO.md by default) — the write side of "#<note>". Returns the file written.
