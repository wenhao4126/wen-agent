...
// (.wenhao/settings.json, only when the project is trusted) and a global
// (~/.wenhao/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".wenhao"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.wenhao/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.wenhao/settings.json.
