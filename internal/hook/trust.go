...
// Trust gates project hooks. A project's .wenhao/settings.json can run
...
// root. The trust flag lives in user-global state (~/.wenhao/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.wenhao/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.wenhao.
...
// TrustPath is ~/.wenhao/trust.json (homeDir overrides ~).
