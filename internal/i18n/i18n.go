...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — wenhao setup
	StepSetKey      string // step 2 command label

	// `wenhao init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // wenhao chat step desc
	StepRunDesc    string // wenhao run step desc
	HelpFooter     string // dim footer linking to wenhao help
	...
	WroteFileFmt          string // "Wrote %s" — used for wenhao.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from wenhao.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: wenhao run [--model NAME] <task>"
...
//
// Priority: override > WENHAO_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"WENHAO_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
