...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `wenhao chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold wenhao.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `wenhao chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `wenhao setup`.",
	StepSetKeyHint: "run `wenhao setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "wenhao help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `wenhao chat`",
	...
	StatusModelPicker:      "↑/↓ move · Enter select · Esc cancel",
	ModelPickTitle:         "Select a model",
	ModelPickHint:          "↑/↓ move · Enter select · Esc cancel",
	ModelPickActive:        "(active)",
	...
	OutputStyleHint:    "set agent.output_style in wenhao.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in wenhao.toml",
	...
	ArgLanguageAuto: "auto-detect from WENHAO_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .wenhao/settings.json (project, after trust) or ~/.wenhao/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in wenhao.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create WENHAO.md in the project root",
	...
	MissingKeyIntro:       "wenhao.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `wenhao setup` or export DEEPSEEK_API_KEY=...), then run `wenhao run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from wenhao.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: wenhao run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing, wrong, or expired. Check the key in .env or run `wenhao setup`.",
	...
	UsageBody: `wenhao — a config- and plugin-driven coding agent (multi-model)

Usage:
  wenhao chat [--model NAME] [-c|--continue] [--resume]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  wenhao run  [--model NAME] [--max-steps N] <task>   run one task and exit
  wenhao serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  wenhao acp [--model NAME]                           serve Agent Client Protocol over stdio (also: wenhao --acp)
  wenhao setup [path]                                 interactive config wizard; writes wenhao.toml (+ .env)
  wenhao config auto-plan [off|on]                    configure automatic plan mode
  wenhao mcp <add|remove|list>                        manage MCP servers in wenhao.toml
  wenhao doctor [--json]                              print redacted local diagnostics
  wenhao version
  wenhao help

Examples:
  wenhao chat
  wenhao chat --continue
...
Configuration:
  Resolution: flag > ./wenhao.toml > ~/.config/wenhao/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'wenhao setup' to scaffold a config; see docs/SPEC.md.
`,
}
