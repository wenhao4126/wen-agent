...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing WENHAO.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "WENHAO.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "WENHAO.md" {
		t.Errorf("should append to the existing WENHAO.md, got %s", got)
	}
