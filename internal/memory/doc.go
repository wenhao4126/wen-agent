// Package memory implements Wenhao's persistent memory. It mirrors Claude
// Code's two-layer model while honoring Wenhao's cache-first architecture:
//
//   - Hierarchical doc memory: WENHAO.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.config/wenhao/WENHAO.md
	ScopeAncestor Scope = "ancestor" // a WENHAO.md above the project root
	ScopeProject  Scope = "project"  // ./WENHAO.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./WENHAO.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// WENHAO.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"WENHAO.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"WENHAO.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
