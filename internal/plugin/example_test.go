...
// buildExamplePlugin compiles cmd/wenhao-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "wenhao-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "wenhao/cmd/wenhao-plugin-example").CombinedOutput()
	if err != nil {
