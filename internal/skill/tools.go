...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to ~/.wenhao/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.wenhao/skills/ (this workspace only); 'global' writes to ~/.wenhao/skills/ (every project)."
	}
