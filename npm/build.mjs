...
for (const t of TARGETS) {
  const name = `@wenhao/cli-${t.node}`;
  const dir = join(STAGE, `cli-${t.node}`);
  const exe = t.goos === "windows" ? "wenhao.exe" : "wenhao";
    ...
    `${JSON.stringify(
      {
        name,
        version,
        description: `wenhao prebuilt binary for ${t.node}.`,
        ...
        repository: {
          type: "git",
          url: "git+https://github.com/esengine/DeepSeek-Wenhao.git",
        },
      },
      null,
      2,
...
const mainDir = join(STAGE, "wenhao");
mkdirSync(mainDir, { recursive: true });
cpSync(join(HERE, "wenhao", "bin"), join(mainDir, "bin"), { recursive: true });
cpSync(join(ROOT, "README.md"), join(mainDir, "README.md"));

const mainPkg = JSON.parse(
  readFileSync(join(HERE, "wenhao", "package.json"), "utf8"),
);
...
// and every prerelease ship under `next` so a bare `npm i wenhao` keeps resolving
// 0.53.x; opt in with `npm i wenhao@next`. (npm rejects `v2` as a tag — it parses
// as a SemVer range.) Promote v2 with a manual `npm dist-tag add wenhao@<ver> latest`.
...
console.log(`publish wenhao@${version} (${distTag})`);
